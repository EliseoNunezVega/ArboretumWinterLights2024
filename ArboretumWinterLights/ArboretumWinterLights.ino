...
void onDmxFrame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t* data)
{
  ...
  if (universe == thisUniverse)
  {
    ...
    if(displayParsedData){
      print("r_channel: "); println(r_channel);
      print("g_channel: "); println(g_channel);
      print("b_channel: "); println(b_channel);
      print("w_channel: "); println(w_channel);
      print("intensity: "); println(intensity);
...
void normalMode(int distance1, int distance2, int howManyHands){
  ...
  uint32_t offset_color = strip.Color(offset_r, offset_g, offset_b, w_channel);
...
void handleSerialCommands(){
  ...
  if (Serial.available() || webInputAvailable) {
    ...
    if (input == 't' || input == 'T'){
      ...
      print("Toggled twoHandEffectActive: "); println(twoHandEffectActive);
    }
...
void loop() {
  ...
  if (printHowManyHands){
    print("How Many Hands: "); println(howManyHandsTouching);
  }
